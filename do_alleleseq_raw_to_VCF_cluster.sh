...
#example run
#perl /net/isi-backup/giuseppe/scripts/P_Various/do_alleleseq_raw_to_VCF.pl -i=/net/isi-scratch/giuseppe/VDR/ALLELESEQ/RESULTS/ENCODE_FILTERED/ASB_EVENTS_BY_SAMPLE/interestingHets_NA06986_EBLfiltered.txt -ucsc_chr -allsites
...
if [ ! $# == 2 ]; then
	...
	echo "NOTE: output will be hg19; edit script to change";
	echo "NOTE: EDIT THE SYM / ASYM OPTION INSIDE BEFORE USAGE";
        exit
fi

PDATA=$1;
ALLSITES=$2;
PCODE="/net/isi-backup/giuseppe/scripts/P_Various/do_alleleseq_raw_to_VCF.pl";
...
for FILE in ${PDATA}/interestingHets*.txt;
	...
	SCRIPT=asb2vcf_${ALLSITES}_${ID}.sh;
        ...
        echo '' >>${PDATA}/${SCRIPT};
	if [ $ALLSITES == 'y' ]; then
		echo "perl ${PCODE} -i=${FILE} -ucsc_chr -allsites" >>${PDATA}/${SCRIPT};
	elif [ $ALLSITES == 'n' ]; then
		echo "perl ${PCODE} -i=${FILE} -ucsc_chr" >>${PDATA}/${SCRIPT};
	else
