...
POUT=${PDATA}/GAT_DistiLD_r${PRAND}_a${DESC};
mkdir ${POUT};

for FILE in ${PDATA}/*.bed;
	do ID=`basename ${FILE} ".bed"`;

        SCRIPT=GAT_DistiLD_${ID}_${PRAND}.sh;
...
#              --nbuckets=2000000            \\
	echo "${PCODE}/gat-run.py     \\
              ...
              --log=${POUT}/GAT_DistilLD_vs_${ID}.log" >> ${PDATA}/${SCRIPT};
        nice -5 qsub  -v "BASH_ENV=~/.bashrc" -e ${POUT}/GAT_DistilLD_vs_${ID}.err -o ${POUT}/GAT_DistilLD_vs_${ID}.tsv -q medium_jobs.q ${PDATA}/${SCRIPT};
        rm ${PDATA}/${SCRIPT};  
done
