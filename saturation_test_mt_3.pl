...
#7/3/2013
#This is obtained from saturation_test_mt2.pl
# the same, but it only uses genetrack with changed parameters (after chat with Yeng)
...
print $outstream "SAMPLE\tMAPPED_READS_BAM\tPEAKS_GENETRACK\tPEAK_RATIO_GENETRACK\n";
my $counter = 1;
while ($INIT <= $MAX){
	my $wc_out; my @wc_out;
	...
	print $outstream  $counter . "\t" .  $number_of_mapped_reads . "\t" .  $genetrack_peaks .  "\t" . ($genetrack_peaks/$total_genetrack_peaks) .  "\n";
...
print $outstream  $counter . "\t" . $total_mapped_reads . "\t" .  $total_genetrack_peaks .  "\t" . ($total_genetrack_peaks/$total_genetrack_peaks) .  "\n";
unlink $infile_bam;
close $outstream;
...
sub run_genetrack{
	...
	system "python $GENETRACK -s 5 -e 10 -F 1 -c $chr $input_file > $temp_file";
    if ( $? == -1 ){
