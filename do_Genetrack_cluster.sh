...
for FILE in ${PDATA}/*.bed;
	...
	#BASEFILE=`basename ${FILE} ".sorted.bed"`;
	SCRIPT=${ID}.sh;

	echo '#!/bin/bash' >>${PDATA}/${SCRIPT};
        echo '' >>${PDATA}/${SCRIPT};
	#echo "python ${PCODE}/genetrack.py -v -s 5 -e 10 -b ${FILE}" >>${PDATA}/${SCRIPT};
	echo "python ${PCODE}/genetrack.py -v -s 5 -e 10 -F 6 -b ${FILE}" >>${PDATA}/${SCRIPT};
        nice -5 qsub -e ${PDATA}/${ID}.err -o ${PDATA}/${ID}.gff -q medium_jobs.q ${PDATA}/${SCRIPT};
	rm ${PDATA}/${SCRIPT}; 
done
