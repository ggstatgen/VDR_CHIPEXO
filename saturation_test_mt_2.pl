...
#5/3/2013
#This is obtained from saturation_test_mt.pl
#With the following differences
#Only do the bowtie alignment once for the full file
#subsample random reads (from the bam instead of the fastq) using picard instead of the htseq-based python function
...
system "rm -rf $full_macs14_basename*"; #remove gem output
system "rm -rf $full_macs2_basename*"; #remove gem output
...
print $outstream "SAMPLE\tMAPPED_READS_BAM\tPEAKS_GENETRACK\tPEAK_RATIO_GENETRACK\tPEAKS_GEM\tPEAK_RATIO_GEM\tPEAKS_MACS14\tPEAK_RATIO_MACS14\tPEAKS_MACS2\tPEAK_RATIO_MACS2\n";
my $counter = 1;
while ($INIT <= $MAX){
	my $wc_out; my @wc_out;
	...
	print $outstream  $counter . "\t" .  $number_of_mapped_reads . "\t" .  $genetrack_peaks .  "\t" . ($genetrack_peaks/$total_genetrack_peaks) .  "\t" . $gem_peaks . "\t" . ($gem_peaks/$total_gem_peaks)  .  "\t" . $macs14_peaks . "\t" . ($macs14_peaks/$total_macs14_peaks) .  "\t" . $macs2_peaks . "\t" . ($macs2_peaks/$total_macs2_peaks) . "\n" ;
...
print $outstream  $counter . "\t" . $total_mapped_reads . "\t" .  $total_genetrack_peaks .  "\t" . ($total_genetrack_peaks/$total_genetrack_peaks) .  "\t" . $total_gem_peaks . "\t" . ($total_gem_peaks/$total_gem_peaks) .    "\t" . $total_macs14_peaks . "\t" . ($total_macs14_peaks/$total_macs14_peaks) .    "\t" . $total_macs2_peaks . "\t" . ($total_macs2_peaks/$total_macs2_peaks) . "\n";
unlink $infile_bam;
close $outstream;
...
sub run_gem{
	...
	system "java -Xmx10G -jar $GEM/gem.jar --g $CHROM_SIZE_PATH --d $GEM/Read_Distribution_ChIP-exo.txt --s $MAPPABILITY --expt $input_file --f SAM --top 0 --t 12 --out $gem_basename";
	if ( $? == -1 ){
