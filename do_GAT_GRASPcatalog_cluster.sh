...
POUT=${PDATA}/GAT_GRASPcatalog_r${PRAND}_a${DESC};
mkdir ${POUT};

for FILE in ${PDATA}/*.bed;
	do ID=`basename ${FILE} ".bed"`;

        SCRIPT=GAT_GRASPcat_${ID}_${PRAND}.sh;
	...
	echo "${PCODE}/gat-run.py     \\
        ...
        nice -5 qsub  -v "BASH_ENV=~/.bashrc" -e ${POUT}/GAT_GRASPcat_vs_${ID}.err -o ${POUT}/GAT_GRASPcat_vs_${ID}.tsv -q medium_jobs.q ${PDATA}/${SCRIPT};
        rm ${PDATA}/${SCRIPT};  
done
