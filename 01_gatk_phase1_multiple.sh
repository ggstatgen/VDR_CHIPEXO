...
#NOTE This is used on merged lane bams which ALREADY have RG information!
#It was created to follow the best practice "Best: per-sample realignment with known indels then recalibration"
#here:
#http://gatkforums.broadinstitute.org/discussion/1186/best-practice-variant-detection-with-the-gatk-v4-for-release-2-0i
#for each sample
...
#2. PICARD - mark duplicates
#3. GATK - create targets to realign
#4. GATK - create realigned BAM
#5. GATK - base recalibrator 
#6. GATK - print reads
...
for FILE in ${PDATA}/*.sam;
	...
	echo '' >>${PDATA}/${SCRIPT};
        echo "java -Xmx4g -jar ${PCODE_GATK}/GenomeAnalysisTK.jar \\
              ...
              -known ${PDATA_GATK}/1000G_phase1.indels.hg19.vcf \\
              -known ${PDATA_GATK}/Mills_and_1000G_gold_standard.indels.hg19.vcf \\
              -o ${PDATA}/${FILE_ID}_md_realigned.bam" >>${PDATA}/${SCRIPT}; 
	echo '' >>${PDATA}/${SCRIPT};
	...
	echo "java -Xmx4g -jar ${PCODE_GATK}/GenomeAnalysisTK.jar \\
              -T BaseRecalibrator \\
              -I ${PDATA}/${FILE_ID}_md_realigned.bam\\
              -L ${INTERVAL} \\
              -nct 8\\
              -et NO_ET \\
              -K ${PKEY} \\
