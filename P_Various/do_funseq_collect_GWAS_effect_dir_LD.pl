...
#like the old one, but pick all VDR-BVs which overlap with PERFECT LD blocks around significant GWAS tag snps.

#PREREQUISITE: do_LD_BEAGLE_mapGWAScat_snps_to_r2_intervals.pl


#the position (hg19) of the sentinel snp in the block in the input bed will be the first item in the 4th column ("__" separated)

#also collect the sentinel snp driving the association.
...
if(!$infile){
     print "USAGE: do_funseq_collect_GWAS_effect_dir_LD.pl -i=<INFILE> -r2=<8|1> -subset=<SUBSET_BED> -title=<TITLE>\n";
     print "<INFILE> [b37] vcf.gz with the variants you want to test (check FIG6_*.vcf.gz files)\n";
     print "<RSQUARESET> one of [8|1]whether to use GWAS snps mapped to LD blocks at r^2=0.8 or r^2=1, respectively\n";
...
my $INPUT_GWAScat_LDmapped;
if($rsquareset eq '8'){
	$INPUT_GWAScat_LDmapped = $INPUT_GWAScat_LDmapped_LD08;
}elsif($rsquareset eq '1'){
	$INPUT_GWAScat_LDmapped = $INPUT_GWAScat_LDmapped_LD1;
}else{
	print STDERR "ERROR: -r2 field $rsquareset not recognised. Aborting..\n";
	exit -1;
}
...
$basename =~ s/(.*)\..*/$1/;
if($subset_bed){
	$output  = $directory . $basename . '_gwas_effectLD' . $rsquareset . $title .  '.Rdata';
}else{
	$output  = $directory . $basename . '_gwas_effectLD' . $rsquareset . '.Rdata';
}
my $input_intersect_gwas = $directory . 'TEMP_gwas_effect_LD_' . $basename . 'data';
...
#1 get intersection
##################
system "$BEDTOOLS intersect -wb -a $infile -b $INPUT_GWAScat_LDmapped > $input_intersect_gwas";
...
#9	start
#10	stop
#11	INFO GWAS. This is __ separated. Split and process to obtain pval, OR, lower_CI, upper_CI, allele freq

#FIELD 11, subfields ( $info_gwas)
#11.0 GWAS SNP POSITION (hg19)
#11.1 rsID #test it's the id of the TAG SNP
#11.2 PUBMED ID
#11.3 AUTHOR
#11.4 date
#11.5 JOURNAL
#11.6 title
#11.7 TRAIT
#11.8 initial sample size
#11.9 rep sample size
#11.10 region
#11.11 gene
#11.12	strongest snp risk allele
#11.13	risk allele frequency - can be NR, not reported
#11.14	p-value
#11.15	?
#11.16	OR
#11.17	95 CI text
...
while(<$instream>){
	...
	my $genomic_coord = $chr_vdrbv . '-' . $pos_vdrbv;
	...
	#==================================
	my @gwas_info = split('__',$info_gwas);

	my $gwas_snp_pos = $gwas_info[0];
	my $gwas_snp = $gwas_info[1]; 
	my $gwas_author = $gwas_info[3];
	my $gwas_trait = $gwas_info[7];
	my $gwas_init_sample = $gwas_info[8];
	my $gwas_rep_sample = $gwas_info[9];
	my $gwas_risk_allele = $gwas_info[12]; #postprocess - typical: rs8080944-A or rs8080944-?
	my $gwas_risk_allele_freq = $gwas_info[13]; #postprocess - typical 0.xxx , NR etc
	my $gwas_pval = $gwas_info[14];
	my $gwas_OR = $gwas_info[16]; #postprocess - typical: 1.45, NR etc
	my $gwas_CI = $gwas_info[17]; #postprocess - typical: [0.xx-1.33](text), NR
	
	print STDERR "$rsid_vdrbv - Warning: OR is not reported\n" if($gwas_OR eq 'NR');
	$gwas_risk_direction = get_gwas_risk_direction($chr_vdrbv,$gwas_snp_pos, $gwas_snp, $gwas_risk_allele,\$gwas_risk_is_reverse_strand);
	if(!$gwas_risk_direction){
		print STDERR "$gwas_snp - Warning: unable to determine risk allele direction. Skipping..\n";
		next;
	}
...
sub get_gwas_risk_direction{
	...
	if($risk_allele eq '?'){
		print STDERR "get_gwas_risk_direction(): risk allele is $risk_allele..\n";
	}

	#$chr =~ s/chr(.*)/$1/; #alleleseq is b37, this is hg19
	my $coord_id = $chr . '-' . $pos;
	
	unless( $variants_1kg{$coord_id}{ANC} &&  $variants_1kg{$coord_id}{DER}  ){
		return undef;
	}
