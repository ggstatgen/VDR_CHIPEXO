...
#In this script I would like to collect and label separately DAFs for CEU/YRI and for the following groups:
#VDR-(hc)BVs/SYM  hitting a VDR-RXR motif
#VDR-(hc)BVs/SYM  hitting an ENCODE motif
#VDR-(hc)BVs/SYM  hitting none of the above
...
#Example: NCENC=TFP(CEBPB|chr5:139639150-139639496),TFP(STAT3|chr5:139638936-139640136),TFP(STAT3|chr5:139638976-139639553),TFP(STAT3|chr5:139638989-139639544),TFP(STAT3|chr5:139638999-139639716)
#you want to check for NCENC=TFM(VDR, if not TFM(.*)
...
if($subset_bed){
	$output  = $directory . $basename . '_DAF_allsites_subsetbed.Rdata';
}elsif($ENH_ONLY){
	$output  = $directory . $basename . '_DAF_allsites_enhonly.Rdata';
}else{
	$output  = $directory . $basename . '_DAF_allsites.Rdata';
}
################
...
while(<$instream>){
	...
	my $LABEL_INMOTIF;# this is either NONE, VDRRXR, ENCODE, BOTH	
	my $info_ncenc;
	my $seen_vdrrxr_motif;
	my $seen_encode_motif;
	...
	if(!$position2sample2readdepth{$genomic_coord}){
		print STDERR "$rs_id - $genomic_coord: ancestral info is not available or ambiguous for this VDR-BV. Skipping..\n";
		next;
	}		
	
	my @info = split(";", $info);
	#I want to flag rows which are in enhancers or are VDR-hcBV
	...
	if($info_ncenc){
		...
		#GET MOTIF CLASS
		my @info_ncenc = split(',', $info_ncenc);
		foreach my $item (@info_ncenc){
			if($item =~ /TFM/){
				unless($item =~ /TFM\(VDR\|VDR/){
					$seen_encode_motif = 1;
				}
			}
		...
		if($seen_encode_motif && $seen_vdrrxr_motif){
			$LABEL_INMOTIF = 'M_BOTH';
		}elsif($seen_encode_motif && !$seen_vdrrxr_motif){
			$LABEL_INMOTIF = 'M_ENCODE';
		}elsif(!$seen_encode_motif && $seen_vdrrxr_motif){
			$LABEL_INMOTIF = 'M_VDRRXR';
		}else{
			$LABEL_INMOTIF = 'NONE';
		}		
	}else{#no ncenc so no enhancer and no motif intersection
		#FILTER: ENHANCER
		next if($ENH_ONLY);
		$LABEL_INMOTIF = 'NONE';
	}
	...
	$data_VDRBV2DAF{$genomic_coord}{LABEL_INMOTIF} = $LABEL_INMOTIF;
}
close $instream;
...
print $outstream "TYPE\tDIRECTION\tETHNICITY\tDAF\tIS_VDRhcBV\tINTERSECTS_MOTIF\n";
...
foreach my $genomic_pos (sort keys %data_VDRBV2DAF ){
    ...
    print $outstream $TYPE . "\t" . $direction . "\t" . 'CEU' . "\t" . $eur . "\t" .  $is_vdrhcbv . "\t" . $data_VDRBV2DAF{$genomic_pos}{LABEL_INMOTIF} . "\n";
    print $outstream $TYPE . "\t" . $direction . "\t" . 'YRI' . "\t" . $afr . "\t" .  $is_vdrhcbv . "\t" . $data_VDRBV2DAF{$genomic_pos}{LABEL_INMOTIF} . "\n";
}
