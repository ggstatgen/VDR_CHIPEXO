...
#13/5/2014
#carry out frequentist regression in a snp-peak association analysis setting using SNPTEST
#https://mathgen.stats.ox.ac.uk/genetics_software/snptest/snptest.html#bayesian_tests

#additionally, compute FDR pval correction for the final results and plot in a manhattan plot (see do_association_SNPTEST_manhattanplot_R.pl for details)
...
#3 matrix of normalised reads out of diffbind
...
#my $exclusion_list = '/net/isi-scratch/giuseppe/VDR/POOLED_4/07_BAM_STAMPY_MIN21/d_macs2_alldup_q0.01/d_bed_RBL/d_diffbind/exclude_list.txt';
...
my $SCRIPT_PATH     = '/net/isi-backup/giuseppe/scripts/P_Various';
my $SNPTEST         = $TOOL_PATH . '/SNPTEST/snptest_v2.5_linux_x86_64_static/snptest_v2.5';
my $RSCRIPT			= $TOOL_PATH . '/R-3.1.0/bin/Rscript';
my $MANHATTAN       = $SCRIPT_PATH . '/do_association_SNPTEST_manhattanplot_R.pl';
...
#-o ./example/ex.out
#-frequentist 1 #frequentist additive model
#-method score
#-pheno pheno
#-range 
#-quantile_normalise_phenotypes #use only on raw read counts?
#-use_raw_phenotypes #I am using DESeq2 normalised phenotypes so I assume no other processing is needed
#etc see instructions

#SNPtest is very slow
#send each peak association analysis to a different cluster node
...
################
#6 gather all result strings in a unique data structure
################
...
#get FDR corrected p-values and manhattan plots
#create R script to do FDR correction
open (my $tempstream,     q{>}, $temp_R_fdr) or die("Unable to open $temp_R_fdr : $!");

print $tempstream 'd <-read.delim( \'' .  $global_out . '\',sep = " ", header=T, check.names=T)' . "\n";
print $tempstream 'pvals = d$frequentist_add_pvalue' . "\n";
print $tempstream 'summary(pvals)' . "\n";
print $tempstream 'padjust <- p.adjust(pvals, method = "fdr", length(pvals))' . "\n";
print $tempstream 'data_adj <- cbind(d,padjust)' . "\n";
print $tempstream 'write.table(data_adj, file=\'' . $global_out_corr . '\',quote=F,sep="\t",col.names=T,row.names=F)' . "\n";

close $tempstream;
system "$RSCRIPT $temp_R_fdr";

#$global_out_corr now contains everything
#run it once on the pval, once on the qval
system "$MANHATTAN -i=$global_out -m=\"pval\"";
system "$MANHATTAN -i=$global_out_corr -m=\"qval\""; #Why is this failing?
