...
#14/12/2015 POST-review. I need  to collect motifbr data for a list of JASPAR motifs, not only the RXRA:VDR heterodimer. Refer to the .doc on google drive to see how I selected these TFs
#Therefore this script will have to be updated to input a motif name to use. I will then call this file via a .sh script
...
if($subset_bed){
	$output  = $directory . $basename . '_' . $motif_name_id . '_concordance_subsetbed.Rdata';
}elsif($ENH_ONLY){
	$output  = $directory . $basename . '_' . $motif_name_id . '_concordance_enhonly.Rdata';
}else{
...
#########################
#4 pick asb/sym snps breaking given motif PWM from funseq
#########################
...
while(<$instream>){
	...
	my $info_motifbr;
	my $info_ncenc;
	#my $info_motifg; try to adapt this for motif gains
	next if($_ =~ /^\#/);
	next if($_ eq '');
	...
	foreach my $item (@motif_change_byTF){
		...
		if($TF_motif_strand =~ /-/){
			...
			$break_position = $TF_snp_position_RC;
		}elsif($TF_motif_strand =~ /\+/){
			#$break_position = 'pos_' . $TF_snp_position;
			$break_position = $TF_snp_position;
		}else{
