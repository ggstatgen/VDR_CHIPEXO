...
#the VDR-hcBV causes the largest magnitude of binding affinity change (median across samples of difference of binding affinity change)
#if more than one VDR-hcBV has same magnitude, collect the one with largest amount of maximum mapped reads.
...
#THOUGHTS:
#There are many ways you could prioritise further these VDR-hcBVs until there is only one of them in the LD block. Please modify the script if you find better ways.
...
my $vcf_line_counter=0;
#save vcf of input variants, p-val thresholded and subset by all,rep, etc
open (my $outstream,  q{>}, $output_fg_vcf) or die("Unable to open $output_fg_vcf: $!");
#vcf header
print $outstream "##fileformat=VCFv4.1\n";
...
if($FOREGROUND_SET eq 'REP'){
	...
	print "fg vcf output file: $vcf_line_counter funseq lines, of which $vcf_pos_counter unique REP variant positions, saved in vcf.\n";
}else{
	foreach my $item (sort keys %fg_vcf){
		print $outstream $item, "\n";
		$vcf_line_counter++;	
	}	
	print "fg vcf output file: $vcf_line_counter funseq lines saved in vcf.\n";
		
}
close $outstream;

my $MHC_vars_count = 0;
open ($outstream,  q{>}, $out_temp_fg_list) or die("Unable to open $out_temp_fg_list: $!");		
...
close $outstream;
print "FG: $MHC_vars_count variants in the MHC discarded.\n";
my $fg_var_number = scalar keys %data_fg;
print "FG: $fg_var_number variants retained.\n";
...
$MHC_vars_count = 0;
open ($outstream,  q{>}, $out_temp_bg_list) or die("Unable to open $out_temp_bg_list: $!");		
...
close $outstream;
print "BG: $MHC_vars_count variants in the MHC discarded.\n";
my $bg_var_number = scalar keys %data_bg;
print "BG: $bg_var_number variants retained.\n";
