...
#Jan 27 2015 Modified to new version of GRASP
#reused because RAM wants R2-based predictions
...
my($basename, $directory) = fileparse($infile_data);
$basename =~ s/(.*)\..*/$1/;
my $output  = $directory . 'GRASPV2_' . $basename . '.csv';


#my $infile_ph_data = '/net/isi-scratch/giuseppe/indexes/GWAS/GRASP/Jan9graspForNCBI';
my $infile_ph_data = '/net/isi-scratch/giuseppe/indexes/GWAS/GRASP/GRASP2final.gz';
...
tie *FILE,   'IO::Zlib', $infile_ph_data, "rb";
while (<FILE>)	{ 
...
#while(<$instream>){
	...
	next if(!$pvalue);
	#TODO adjust here, ad hoc code
	if($pvalue eq '8.36E?12'){ #8.36E?12
		$pvalue = '8.36E-12';
	}
	...
	#REMOVE THIS?
	next if($phenotype =~ /^Methylation levels/); #remove methylation?
	#REMOVE THIS? Differential exon level
	next if($phenotype =~ /^Differential exon level/); #remove methylation?
	#Differential splicing
	next if($phenotype =~ /^Differential splicing/); 
	next if($ph_description =~ /^Metabolites/);
	#next if($ph_description =~ /^Urate (in serum)/);
	#next if($ph_description =~ /^Uric acid levels, in serum/);
...
#		next if($phenotype =~ /^Differential exon level/); 
#		next if($phenotype =~ /^Differential splicing/);
#		next if($ph_description =~ /^Metabolites/);
#		#next if($ph_description =~ /^Urate (in serum)/);
#		#next if($ph_description =~ /^Uric acid levels, in serum/);
