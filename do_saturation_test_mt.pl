...
#OUTPUTS
#TSV FILE  <number of reads> <frequency> <macs14_peaks> <macs2_peaks> <Genetrack_peaks>


my $infile;
my $outfile;
my $input_id; # to customise randomised outputs
...
if(!$infile){
     print "USAGE: do_saturation_test.pl -input=<INFILE> -output=<OUTFILE> -step=<STEP>\n";
     print "<INFILE> input fastq file\n";
     print "<OUTFILE> output TSV file\n";
...
#temp data
#---------
my $random_fastq_file       = $input_id  . '_random.fq'; #the python fastq sumbsampler writes here / bowtie reads
my $random_sam_file         = $input_id  . '_random.sam'; #bowtie writes here / picard reads
my $random_bam_file         = $input_id  . '_random.bam'; #picard writes here /macs2 and GEM read
my $random_bai_file         = $input_id  . '_random.bai'; #picard writes here
my $random_bed_file         = $input_id  . '_random.bed'; #bamtobed writes here / genetrack reads
my $random_macs2_basename   = $input_id  . '_random_macs2'; #macs2 outputs in these
my $random_macs14_basename  = $input_id  . '_random_macs14'; #macs14 outputs in these
my $random_gem_basename     = $input_id  . '_random_gem'; #gem outputs in these
...
while ($INIT <= $MAX){
	...
	#----------------
	print "Creating random .fastq file..\n";
	...
	@wc_out = split(/ /, $wc_out);
	my $number_of_reads = $wc_out[0]/4;	
        print '------Random .fastq sample number ' . $counter . ' has ' . $number_of_reads . ' entries' . "\n";
	
	#--------------
	#align
	#-------------
	print "Aligning random .fastq file..\n";
	#${PCODE}/bowtie-0.12.9/bowtie -q -S --best --strata -m 1 -p 10 --chunkmbs 1024 ${PINDEX} ${FILE} ${PDATA}/${BASEFILE}.sam
	...
	#$wc_out = `wc -l $random_sam_file`;
        #@wc_out = split(/ /, $wc_out);
        #my $number_of_sam_reads = $wc_out[0];   
	#print '------Random .sam sample number ' . $counter . ' has ' . $number_of_sam_reads . ' entries' . "\n";
	...
	for my $i (0 .. $#HUMAN_CHRS) {
		push(@gt_threads, threads->create(\&run_genetrack, $HUMAN_CHRS[$i]));
	}
	...
	print $outstream  $counter . "\t" . $number_of_reads . "\t" .  $genetrack_peaks .  "\n";
	...
	system "rm $input_id*chr*.gff"; #rm genetrack outputs
	system "rm $input_id*chr*.bed"; #rm genetrack outputs
...
sub run_genetrack{
	my ( $chr ) = @_; 
	my $temp_file              = $input_id .  '_'  . $chr . '_' .  'random_raw.gff';
	my $random_gff_file        = $input_id .  '_'  . $chr . '_' .  'random.gff';
	my $random_sorted_gff_file = $input_id .  '_'  . $chr . '_' .  'random_sorted.gff';
	my $random_sorted_bed_file = $input_id .  '_'  . $chr . '_' .  'random.bed'; # this is the one where you need to count

	#python ${PCODE}/genetrack.py -v -s 5 -e 10 -F 6 -c chr1 ${FILE}
	system "python $GENETRACK -s 5 -e 10 -F 6 -c $chr $random_bed_file > $temp_file";
    if ( $? == -1 ){
