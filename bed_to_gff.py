...
#GIUSEPPE I found this somewhere and modified the attribute content because HT doesn't work with it
#the following is a line that works with htseq
#13_random	protein_coding	exon	10961	11040	.	+	.	ID=exon:ENSGALT00000001782:3;Parent=ENSGALT00000001782;
#the feature in column 3, exon, has to be specified as id in the attribute, followed by an attribute id
#so
#chr1      protein_coding  peak    10961   11040   .       +       .       ID=peak:peak1;
import sys
    ...
    for i, line in enumerate( file( input_name ) ):
        ...
        if line and not line.startswith( '#' ) and not line.startswith( 'track' ) and not line.startswith( 'browser' ):
            try:
                ...
                try:
                    #group = elems[3]
                    group = 'ID=exon:ENSGALT00000001782:%d' % (i + 1)
                    #group = 'TAG=%d' % ( i + 1) 
                except:
                    group = 'group%d' % ( i + 1 )
                if complete_bed:
