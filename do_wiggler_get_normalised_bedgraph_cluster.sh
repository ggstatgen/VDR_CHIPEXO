...
for FILE in ${PDATA}/*.bam;
	...
	SCRIPT=align2raw_${ID}.sh;

	echo '#!/bin/bash' >>${PDATA}/${SCRIPT};
	echo 'source /home/giuseppe/.bashrc' >>${PDATA}/${SCRIPT};
	echo "${PCODE}/align2rawsignal -i=${FILE} -s=${PSEQ} -u=${PMAP} -o=${PDATA}/align2rawsignal_${ID}.bg -of=${OUTF} -v=${PDATA}/align2rawsignal_${ID}.log -mm=4" >>${PDATA}/${SCRIPT};

	#add a bedgraph header
	awk 'BEGIN{print "new line"}{print}' infile > outfile

	nice -5 qsub -e ${PDATA}/align2rawsignal_${ID}.err -o ${PDATA}/align2rawsignal_${ID}.out -q newnodes.q ${PDATA}/${SCRIPT};
	rm ${PDATA}/${SCRIPT};
done
